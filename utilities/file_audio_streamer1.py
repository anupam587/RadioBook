...
def make_wav_header(rate: int, channels: int, sampwidth: int,
                    num_frames: int = 0x7FFFFFFF) -> bytes:
    """
    Build a WAV header. Use a large num_frames so the Content-Length appears
    'infinite' to media players — required for proper HTTP streaming.
    """
    buf = io.BytesIO()
